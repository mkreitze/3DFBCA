...
sMs=[[0.320205,0.952292,0.351335,0.837774,0.390741,0.728013,0.614486,0.378596,0.320205,0.952292,0.351335,0.837774,0.390741,0.728013,0.614486,0.378596],[0.390741,0.728013,0.614486,0.378596,0.320205,0.952292,0.351335,0.837774,0.390741,0.728013,0.614486,0.378596,0.320205,0.952292,0.351335,0.837774]]
...
#COMMENT CORRECTLY
#I hard coded it ;( 
def genMtl(directory,quanti):
    mtlRend= open(directory+str(quanti)+".mtl",'x')
    ...
    return()
#comment correctly
def genObj(fBCA,updateNum=0,fBCANUM=0,directory=""):
    QUANTIFER="sM"+str(fBCANUM)+"Gen"+str(updateNum)
    objRend= open(directory+str(QUANTIFER)+".obj",'x')
    objRend.write("# "+str(QUANTIFER)+" OBJ RENDER \n")
    objRend.write("mtllib "+"./"+str(QUANTIFER)+".mtl"+"\n")
    genMtl(directory,QUANTIFER)
    cubeNum=0
    for x in range(fBCA.length):
        for y in range(fBCA.width):
            for z in range(fBCA.height):
